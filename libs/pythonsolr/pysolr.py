...
class Results(object):
    def __init__(self, response=None,decoder=None):
        ...
        if result.get('highlighting'):
            self.highlighting = result['highlighting']
        
        if result.get('facet_counts'):
            self.facets = result['facet_counts']
        
        if result.get('spellcheck'):
            self.spellcheck = result['spellcheck']
        
        self.docs = result['response']['docs']
        self.hits = result['response']['numFound']
...
class Solr(object):
    ...
    def _select(self, params):
        ...
        if 'fq' in params: params['fq'] = self._encode_q(params['fq'])
        if 'spellcheck.q' in params: params['spellcheck.q'] = self._encode_q(params['spellcheck.q'])
    ...
    def search(self, q, **kwargs):
        ...
        return Results(response,decoder=self.decoder)
