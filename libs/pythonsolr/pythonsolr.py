...
class SolrResultsPaginator(object):
    ...
    def __unicode__(self):
        fmt = "SolrResultsPaginator(hits={hits}, solr={solr}, query={query}, default_params={default_params})"
...
class PythonSolr(Solr):
    ...
    def search(self, q, **kwargs):
        ...
        log.debug("ParselySolr: starting Solr search with query={query} and params={params}".format(
        ...
        try:
            ...
            for item in self.batch:
                try:
                    ...
                    log.error(u"Could not add item to solr index, title: {title}".format(title=itemtitle))
                    log.exception(u"Exception stack trace for adding item id #{itemid}".format(itemid=itemid))
            if auto_commit:
                self.commit()
